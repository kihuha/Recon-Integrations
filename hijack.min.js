...
(function () {
  ...
  1: [function (require, module, exports) {
    ...
    const ENABLED_INTEGRATIONS = [{"name":"ga","options":{"api_key":"UA-158757229-1","custom_dimension":1}},{"name":"gtm"},{"name":"mixpanel"},{"name":"heap"},{"name":"segment"},{"name":"munchkin"},{"name":"facebook-pixel"},{"name":"amplitude"},{"name":"fullstory"},{"name":"crazyegg","options":{"labelBlockedIndex":1}}];
  ...
  2: [function (require, module, exports) {
    ...
    const labelBlocked = function () {
      const instanceNames = Object.keys(window.amplitude._instances);
      const interval = setInterval(function () {
        const amp = window.amplitude;

        if (amp) {
          let markAsBlocked = new amplitude.Identify().set(_constants.USER_UNBLOCKED_PROPERTY, _constants.USER_UNBLOCKED_PROPERTY_VALUE);

          for (const instanceName of instanceNames) {
            if (instanceName !== '$default_instance') {
              amplitude.getInstance(instanceName).identify(markAsBlocked);
              amplitude.getInstance(instanceName).logEvent(_constants.USER_UNBLOCKED_PROPERTY);
            } else {
              amplitude.getInstance().identify(markAsBlocked);
              amplitude.getInstance().logEvent(_constants.USER_UNBLOCKED_PROPERTY);
            }
          }
  ...
  3: [function (require, module, exports) {
    ...
    const isBlocked = async function () {
      try {
        const response = await fetch("https://recording.crazyegg.com/s");
    ...
    const labelBlocked = function () {
      const crazyeggConfig = _inputs.ENABLED_INTEGRATIONS.filter(config => config.name === name)[0];

      const labelBlockedIndex = crazyeggConfig.options.labelBlockedIndex;
      const interval = setInterval(() => {
        if (isLoaded() && !!labelBlockedIndex) {
          CE2.set(labelBlockedIndex, "blocked");
          clearInterval(interval);
        }
      }, 100);
    };

    const apiHosts = ["crazyegg.com"];
  ...
  5: [function (require, module, exports) {
    ...
    const isBlocked = async function () {
      try {
        const res1 = await fetch('https://rs.fullstory.com/rec/bundle', {
          headers: {
            accept: '*/*',
        ...
        const res2 = await fetch('https://edge.fullstory.com/s/fs.js', {
          referrerPolicy: 'no-referrer-when-downgrade',
          body: null,
          method: 'GET',
          mode: 'cors',
        ...
        if (res1.status == 206 && res2.status == 200) return false;
        return true;
      } catch (e) {
        return true;
      }
    ...
    const isLoaded = function () {
      return !!(window && window._fs_loaded);
    };
    ...
    const apiHosts = ['fullstory.com'];
    const cdnHosts = ['fullstory.com'];
    const polyfillBuster = undefined;
    module.exports = {
      name,
      isBlocked,
  ...
  6: [function (require, module, exports) {
    ...
    const labelBlocked = function () {
      ...
      if (config.options && config.options.api_key && config.options.custom_dimension && gaObjectName && window[gaObjectName]) {
        const gaObject = window[gaObjectName];
        const index = gaObject.q.findIndex(element => element[0] === 'create' && element[1] === config.options.api_key);

        if (index !== -1) {
          const found = gaObject.q[index];
          let setKey = 'set';

          if (typeof found[2] === 'object' && 'name' in found[2]) {
            setKey = `${found[2]['name']}.set`;
          }

          gaObject(setKey, `dimension${config.options.custom_dimension}`, 'true');
          let args = gaObject.q.pop();
          gaObject.q.splice(index + 1, 0, args);
        }
      }
  ...
  9: [function (require, module, exports) {
    ...
    var _fullstory = _interopRequireDefault(require("./fullstory"));
  ...
  10: [function (require, module, exports) {
    ...
    const USER_UNBLOCKED_PROPERTY = 'unblocked';
    const USER_UNBLOCKED_PROPERTY_VALUE = 'true';
    module.exports = {
      USER_UNBLOCKED_PROPERTY,
      USER_UNBLOCKED_PROPERTY_VALUE
    };
  }, {}],
      ...
      const interval = setInterval(function () {
        if (isLoaded()) {
          window.analytics.identify(window.analytics.user().id(), {
            [_constants.USER_UNBLOCKED_PROPERTY]: _constants.USER_UNBLOCKED_PROPERTY_VALUE
          }, {
            anonymousId: window.analytics.user().anonymousId()
          });
          window.analytics.track(_constants.USER_UNBLOCKED_PROPERTY);
          clearInterval(interval);
        }
      }, 10);
    };
